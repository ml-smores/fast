...
public class Linear {
	 ...
	 *          predicted classes
	 */
	public static void crossValidation(Problem prob, Parameter param,
	...
	/**
	 * Loads the model from inputReader. It uses {@link java.util.Locale#ENGLISH}
	...
	public static Model loadModel(File modelFile) throws IOException {
		BufferedReader inputReader = new BufferedReader(new InputStreamReader(
				new FileInputStream(modelFile), FILE_CHARSET));
		try {
	 ...
	 */
	public static double predictProbability(Model model, Feature[] x,
			double[] prob_estimates) throws IllegalArgumentException {
	...
	public static double predictValues(Model model, Feature[] x,
			double[] dec_values) {
		int n;
		if (model.bias >= 0)
	...
	static void printf(Formatter formatter, String format, Object... args)
			throws IOException {
	 ...
	 * </p>
	 */
	public static void saveModel(Writer modelOutput, Model model)
			throws IOException {
	...
	public static void saveModel(File modelFile, Model model) throws IOException {
		BufferedWriter modelOutput = new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream(modelFile), FILE_CHARSET));
	 ...
	 * </pre>
	 */
	private static void solve_l2r_l1l2_svc(Problem prob, double[] w, double eps,
			double Cp, double Cn, SolverType solver_type) {
		int l = prob.l;
		int w_size = prob.n;
		int i, s, iter = 0;
		double C, d, G;
	 ...
	 */
	private static void solve_l2r_l1l2_svr(Problem prob, double[] w,
			Parameter param) {
		int l = prob.l;
		double C = param.C;
		double p = param.p;
		...
		while (iter < max_iter) {
			...
			for (s = 0; s < active_size; s++) {
				...
				beta[i] = Math.min(Math.max(beta[i] + d, -upper_bound[GETI_SVR(i)]),
						upper_bound[GETI_SVR(i)]);
				d = beta[i] - beta_old;
		...
		v = 0.5 * v;
		for (i = 0; i < l; i++) {
			v += p * Math.abs(beta[i]) - y[i] * beta[i] + 0.5 * lambda[GETI_SVR(i)]
					* beta[i] * beta[i];
			if (beta[i] != 0)
				nSV++;
		}
	 ...
	 * @since 1.7
	 */
	private static void solve_l2r_lr_dual(Problem prob, double w[], double eps,
			double Cp, double Cn) {
		int l = prob.l;
		int w_size = prob.n;
		int i, s, iter = 0;
		...
		v *= 0.5;
		for (i = 0; i < l; i++)
			v += alpha[2 * i] * Math.log(alpha[2 * i]) + alpha[2 * i + 1]
					* Math.log(alpha[2 * i + 1]) - upper_bound[GETI(y, i)]
	 ...
	 * @since 1.5
	 */
	private static void solve_l1r_l2_svc(Problem prob_col, double[] w,
		...
		while (newton_iter < max_newton_iter) {
			...
			for (num_linesearch = 0; num_linesearch < max_num_linesearch; num_linesearch++) {
				...
				for (int i = 0; i < l; i++) {
					...
					cond += C[GETI(y, i)]
							* Math.log((1 + exp_wTx_new[i]) / (exp_xTd + exp_wTx_new[i]));
				}
	...
	static Problem transpose(Problem prob) {
		...
		// hy:
		if (prob.W != null && prob.W.length > 0)
	...
	public static Model train(Problem prob, Parameter param) {
		...
		if (prob.W != null && prob.W.length > 0) {
			...
			prob = removeNonpositiveWeights(newProb, prob);
		}
		// }hy
		...
		if (param.solverType == SolverType.L2R_L2LOSS_SVR || //
			...
			for (int i = 0; i < param.getNumWeights(); i++) {
				...
				if (j == nr_class)
					throw new IllegalArgumentException("class label "
	...
	private static void checkProblemSize(int n, int nr_class) {
		if (n >= Integer.MAX_VALUE / nr_class || n * nr_class < 0) {
			throw new IllegalArgumentException(
					"'number of classes' * 'number of instances' is too large: "
							+ nr_class + "*" + n);
		}
	}
	...
	private static void train_one(Problem prob, Parameter param, double[] w,
		...
		// hy:
		// System.out.println("SolverType: " + param.solverType);
	...
	// hy{
	public static Problem removeNonpositiveWeights(Problem newProb, Problem prob) {
		int i;
		int l = 0;
		for (i = 0; i < prob.l; i++)
			if (prob.W[i] > 0)
				l++;
		newProb = new Problem(prob);
		newProb.l = l;
		newProb.x = new Feature[l][];
		// TODO: check pass by reference
		newProb.y = new double[l];
		...
		for (i = 0; i < prob.l; i++)
			if (prob.W[i] > 0) {
				// TODO: check pass by reference
				newProb.x[j] = Arrays.copyOf(prob.x[i], prob.x[i].length);
				newProb.y[j] = prob.y[i];
				newProb.W[j] = prob.W[i];
				j++;
			}
		// else
		// System.out.println("Remove the current instance!");
		return newProb;
	}
