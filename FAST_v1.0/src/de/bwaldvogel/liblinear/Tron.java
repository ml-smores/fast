...
class Tron {
	...
	void tron(double[] w) {
		...
		while (iter <= max_iter && search != 0) {
			...
			// hy*
			// info(
			// "iter %2d act %5.3e pre %5.3e delta %5.3e f %5.3e |g| %5.3e CG %3d%n",
	...
	private int trcg(double delta, double[] g, double[] s, double[] r) {
		...
		while (true) {
			...
			if (euclideanNorm(s) > delta) {
				...
				opts.cgReachesTrustRegionBoundary = true;
				alpha = -alpha;
				daxpy(alpha, d, s);
	...
	private static double euclideanNorm(double vector[]) {
		...
		// this algorithm is (often) more accurate than just summing up the squares
		// and taking the square-root afterwards

		double scale = 0; // scaling factor that is factored out
		double sum = 1; // basic sum of squares from which scale has been factored
										// out
		for (int i = 0; i < n; i++) {
			if (vector[i] != 0) {
