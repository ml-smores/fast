...
/**
 * @infile for getFilesInSpecifiedDir: ends with specified surfix, 
 *also use modelName_prediction to name it so that the modelName could be extracted (by use nameExtractor, and get the first element)
 * @infile for evaluateClassifier: actualLabel, predictLabel, predictProb
 * @infile by luck, now can deal with XX_test_0.5.txt, because of the first point in 0.5 -> extract to fold 0
 */
...
public class EvaluationGeneral {
	...
	public static double evaluateOnMultiFiles(String modelName_, int numRuns_,
			int numFolds_, String outDirForComparingModels, String inOutDir,
			String outFile, String standardFileSurfix) {
		// outFileNameAllModel= "all-models-evaluation-metrics" + outFileSurfix
		// loggerOutFileName = "evaluation.log";
	...
	public static double run(String modelName, int numRuns, int numFolds,
		...
		try {
			...
			getFilesInSpecifiedDir(inOutDir, predFileSurfix, modelNameExtractor,
					numRuns, numFolds, fileNames, modelNames);
	...
	public static void getFilesInSpecifiedDir(String specifiedDir, String surfix,
			...
			throws IOException {
		printAndLog("\ngetFilesInSpecifiedDir(): " + specifiedDir + "\n");
