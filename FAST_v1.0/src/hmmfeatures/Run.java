/**
 * @toImprove
 * -- the readOneHmmOneTime mode suppose 1)train and test are both ordered by skill, student, time, 2) test set always have same amount of skill as train set
 * -- for readAllHmms, need to support output prediction on train
 * 
 * @possibleErrorInCode:
 * -- in BaumWelchScaledLearner, the other estimateGammas
 * -- in ForwardBackwardScaledCalculator, don't understand why do computeProbability() in that way?
 * 
 * @extremeCases:
 * 	-- if there remains no datapoints in training (due to remove seq length = 1), then return the random hmm;
 *  -- if the LBFGS part breaks (has RuntimeException), then run HMMs instead (non paramterizing).
 *  
 * @infile format: 
 *   -- records should be ordered by user, time (allow not ordered by skill, but need same users' same skill records always put together because of StudentList.java's mechanism(use current studentid compared with prestudentid to decide split into new sequence)
 *   -- never put bias feature in the input file, since bias feature is added by my Opts class
 *   -- for the outcome column, should be "incorrect" or "correct", I hard coded using them...
 *   -- use "feature_NAME" to denote feature column (code: String featureName = columns[i].replace("feature_", ""))
 *   -- when reading features from input by DataPointList class, only read and create data structures with real meaningful features (never create any bias featues (not even 1biasf) here) => only create the real feature vector for computing probablistic later
 * 	 -- if use addSharedStuDummyFeatures or addSharedItemDummyFeatures, then userid column and problem column should use different naming system (since i their name is directly used as feature names and be put into Bijection, if they overlap, Bijection will consider them the same)
 *   -- if "inputProvideFeatureColumns=false", then need to check whether train and test files have same feature columns; otherwise, we assume that train and test feature columns are the same (same # and same order)
 * 
 * 
 * @splitForHmms currently use KCs column to split (only support one KC per record now)
 */
