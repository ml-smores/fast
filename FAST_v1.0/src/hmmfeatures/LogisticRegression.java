...
public class LogisticRegression {
	...
	public double[] train() {
		...
		if (opts.LBFGS)
			...
			featureWeights = useLiblinear(features, labels, expectedCounts);
	...
	public double[] useLiblinear(double[][] features, int[] outcomes,
			double[] expectedCounts) {
		double[] targets = intToDoubleArray(outcomes);
		// swap targets to make 1 ("correct") appears first, so that Cp(which
		// corresponds to prob.y>0 and weighted_C[0](which corresponds to the first
		// appearing label)) can always correspond to 1 ("correct"))
		int weightIndex = 0;
		for (; weightIndex < targets.length; weightIndex++) {
			if (expectedCounts[weightIndex] != 0)
				break;
		}
		if (targets[weightIndex] != opts.obsClass1 * 1.0) {
			for (int k = weightIndex + 1; k < targets.length; k++)
				if (targets[k] == opts.obsClass1 && expectedCounts[k] != 0) {
					// targets[k] == opts.obsClass1
					swap(targets, 0, k);
					if (opts.useInstanceWeightToTrainParamterezdEmit)
						swap(expectedCounts, 0, k);
					swap(features, 0, k);
					break;
				}
		}

		featureWeights = new double[featureMapping.getSize()];
		// double addBias = -1.0;
		// always rely on putting bias when reading datapoints
		Train liblinearTrain = new Train();

		if (opts.useInstanceWeightToTrainParamterezdEmit) {
			String[] options = { "-s", "0", "-B", "-1.0", "-c",
					opts.LIBLINEAR_C_PENALTY + "" };
			// System.out.println(opts[0] + opts[1] + opts[2] + opts[3]);
			featureWeights = liblinearTrain.run(options, features, targets,
					expectedCounts);
		}
		else if (opts.useClassWeightToTrainParamerizedEmit) {

			// -w0: It only requires -wi's i be the real target variable value.
			String[] options = { "-s", "0", "-B", "-1.0", "-w0",
					expectedCounts[0] + "", "-w1", expectedCounts[1] + "", "-c",
					opts.LIBLINEAR_C_PENALTY + "" };
			// System.out.println(opts[0] + opts[1] + opts[2] + opts[3]);
			featureWeights = liblinearTrain.run(options, features, targets);
		}
