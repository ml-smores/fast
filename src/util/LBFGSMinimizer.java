...
public class LBFGSMinimizer implements GradientMinimizer, Serializable {
	...
	public LBFGSMinimizer(Opts opts) {
		this.opts = opts;
	}
	...
	public double[] minimize(DifferentiableFunction function, double[] initial,
		...
		for (int iteration = 0; iteration < maxIterations; iteration++) {
			...
			// hy:
			if (direction == null && opts.ensureStopForLBFGS) {
				// 1) return initial
				Logger.err("\tWARN: return initial guess!");
				// 2) re-initialize feature weights, recalucuate initial NLL and restart
				return initial;
			}
		...
		for (int i = historySize() - 1; i >= 0; i--) {
			...
			if (rho[i] == 0.0) {
				if (opts.ensureStopForLBFGS) {
					...
					opts.parameterizedEmit = false;// use just EM to retrain
					return null;
				}
				else
