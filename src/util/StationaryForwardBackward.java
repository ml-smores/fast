...
public class StationaryForwardBackward {
	...
	public void setInput(StationarySequenceInstance seqInstance, boolean viterbi) {
		assert seqInstance.getSequenceLength() < seqModel
				.getMaximumSequenceLength();
		...
		for (int pos = 1; pos < obsLength; ++pos) {
			for (int state = 0; state < numStates; ++state) {
				...
				for (int beforeStateIndex = 0; beforeStateIndex < beforeStates.length; ++beforeStateIndex) {
					...
					double delta = alphas[pos - 1][beforeState]
							* edgeBackwardPotentials[state][beforeStateIndex]
							* nodePotentials[pos][state];
					if (delta > max) {
						argMax = beforeState;
						max = delta;
					}
	...
	// hy:edgeBackwardPotentials: aij
	// nodePotentials: bjk(current)
		...
		for (int pos = 0; pos < obsLength; ++pos) {
			...
			if (pos == 0) {
				for (int state = 0; state < numStates; ++state) {
					...
					if (alphas[pos][state] > max)
						max = alphas[pos][state];
				}
			}
			else {
				for (int state = 0; state < numStates; ++state) {
					...
					if (nodePotentials[pos][state] > 0) {
						...
						for (int beforeStateIndex = 0; beforeStateIndex < beforeStates.length; ++beforeStateIndex) {
							int beforeState = beforeStates[beforeStateIndex];
							alpha += alphas[pos - 1][beforeState]
									* edgeBackwardPotentials[state][beforeStateIndex];
						}
			...
			if (max == 0.0 || Double.isInfinite(max)) {
				throw new RuntimeException(String.format("The alphas[%d] has max=%.3f",
						pos, max));
			}
			...
			while (max > SCALE) {
				...
				scale *= SCALE;
				logScale += 1;
				if (logScale > MAX_LOGSCALE)
					throw new RuntimeException("Max log scale exceeded.");
			}
	...
	// hy: edgeBackwardPotentials, edgeForwardPotentials: aij
	// nodePotentials: bjk(current)
				...
				for (int state = 0; state < numStates; ++state) {
					...
					if (nodePotentials[pos][state] > 0.0) {
						...
						for (int nextStateIndex = 0; nextStateIndex < nextStates.length; ++nextStateIndex) {
							...
							beta += edgeForwardPotentials[state][nextStateIndex]
									* betas[pos + 1][nextState];
						}
	...
	// edgePosteriors: P(qi->qj|Dd,t)
	// nodePosteriors: P(qi|Dd,t)
	private void computePosteriors() {
		...
		if (z == 0.0) {
			throw new RuntimeException(
					"Forward-Backward: No non-zero label sequences");
		}
		;
		...
		for (int i = 0; i + 1 < obsLength; i++) {
			...
			assert Math.abs(posterior_scale) <= 3.0 : "Exp scale is "
					+ posterior_scale;
			for (int s = 0; s < numStates; s++) {
				...
				for (int tIndex = 0; tIndex < nextLabels.length; ++tIndex) {
					...
					if (beta_t == 0.0)
						continue;
					double unscaled_posterior = x * edgeForwardPotentials[s][tIndex]
							* beta_t;
		...
		for (int t = 0; t < numStates; t++) {
			...
			// double unscaled_posterior = alpha_t * beta_t / (z *
			// nodePotentials[obsLength-1][t]);
	...
	private void probCheck() {
		...
		for (int pos = 0; pos < obsLength; ++pos) {
			double nodeSum = 0.0;
			double edgeSum = 0.0;
			for (int l = 0; l < nodePosteriors[pos].length; ++l)
				nodeSum += nodePosteriors[pos][l];
			if (relativeDiff(1.0, nodeSum) > 1.0e-2) {
				throw new RuntimeException("Node Sum: " + nodeSum + " not 1.0 for "
						+ pos);
			}
		}
		double edgeSum = 0;
		...
		if (relativeDiff(obsLength - 1.0, edgeSum) > 1.0e-2) {
			throw new RuntimeException("Failed ProbCheck: Edge Sum: " + edgeSum
					+ " for " + obsLength);
		}
	}
