...
/**
 * Represents all the settings and output of an execution of a program. An
 * execution is defined by all the options registered with OptionsParser.
...
public class Execution {
	...
	static OrderedStringMap inputMap = new OrderedStringMap(); // Accessed by
																															// monitor thread
	...
	/**
	 * Return an unused directory in the execution pool directory. Set
	 * virtualExecDir
	 */
	public static String createVirtualExecDir() {
		if (useStandardExecPoolDirStrategy) {
			...
			actualExecPoolDir = new File(SysInfoUtils.getcwd(), "state/hosts/"
					+ SysInfoUtils.getShortHostName()).toString();
		...
		if (!StrUtils.isEmpty(execPoolDir) && !new File(execPoolDir).isDirectory())
			throw Exceptions.bad("Execution pool directory '" + execPoolDir
					+ "' doesn't exist");
		if (!StrUtils.isEmpty(actualExecPoolDir)
				&& !new File(actualExecPoolDir).isDirectory())
			throw Exceptions.bad("Actual execution pool directory '"
					+ actualExecPoolDir + "' doesn't exist");

		if (!StrUtils.isEmpty(execDir)) { // Use specified execDir
			boolean exists = new File(execDir).isDirectory();
			if (exists && !overwriteExecDir)
				throw Exceptions
						.bad("Directory already exists and overwrite flag is false");
			if (!exists)
				...
				// execution pool directory requested. In fact, we always do this for
				// simplicity.
				String oldActualExecDir = Utils.systemGetStringOutputEasy("readlink "
						+ execDir);
				if (oldActualExecDir == null) { // Not symlink
					...
					// Note that now the execution numbers might not correspond between
					// the
					...
					Utils.systemHard(String.format("mv %s %s", oldActualExecDir,
							newActualExecDir));
					// Update the symlink (execDir -> newActualExecDir)
					Utils.systemHard(String.format("ln -sf %s %s",
							newActualExecDir.getAbsolutePath(), execDir));
				}
			}
			return virtualExecDir = execDir;
		}

		// execDir hasn't been specified, so we need to pick one from a pool
		// directory
		...
		for (int i = 0; numFailures < 3; i++) {
			...
			// Actual file
			File g = StrUtils.isEmpty(actualExecPoolDir) ? null : new File(
					actualExecPoolDir, i + ".exec");
			...
			if (!files.contains(i + ".exec") && (g == null || !g.exists())) {
				...
				// Create symlink before mkdir to try to reserve the name and avoid race
				// conditions
	...
	public static void init(String[] args, Object... objects) {
		// // Parse options
		// If one of the objects is an options parser, use that; otherwise, create a
		// new one
		...
		// These options are specific to the execution, so we don't want to
		// overwrite them
		// with a previous execution's.
		parser.setDefaultDirFileName("options.map");
		parser.setIgnoreOptsFromFileName("options.map", ListUtils.newList(
				"log.file", "exec.execDir", "exec.execPoolDir", "exec.actualPoolDir",
				"exec.makeThunk"));
		...
		if (!makeThunk)
			Record.init(!StrUtils.isEmpty(recordPath) ? recordPath : Execution
					.getFile("record"));
	}
 ...
 */
	private static void initializeJars(boolean inVirtualExecDir) {
		if (jarFiles.size() > 0) {
			...
			for (String jarFile : jarFiles) {
				// Load classes
				String jarPath = inVirtualExecDir ? new File(virtualExecDir, new File(
						jarFile).getName()).getPath() : jarFile;
	...
	public static void finish() {
		if (!makeThunk) {
			...
			if (LogInfo.getNumErrors() > 0 || LogInfo.getNumWarnings() > 0)
				stderr.printf("%d errors, %d warnings\n", LogInfo.getNumErrors(),
						LogInfo.getNumWarnings());
			if (startMainTrack)
				end_track();
		}
	...
	public static void runWithObjArray(String[] args, Object[] objects) {
		...
		if (makeThunk) {
			...
			// Set classpath to make sure we have the exact same environment when we
			// run the thunk
			cmd.add("-cp " + StrUtils.join(jarFiles, ":") + ":"
					+ System.getenv("CLASSPATH"));
			// java.class.path doesn't pick up $CLASSPATH for scala programs
			// cmd.add("-cp " + StrUtils.join(jarFiles,
			// ":")+":"+System.getProperty("java.class.path"));
			cmd.addAll(ListUtils.newList(thunkMainClassName == null ? mainObj
					.getClass().getName() : thunkMainClassName, "++" + virtualExecDir
					+ "/options.map", // Load these options
					// Next time when we run, just run in the same path that we used to
					// create the thunk
					"-execDir", virtualExecDir, "-overwriteExecDir"));
			IOUtils.printLinesHard(
					Execution.getFile("job.map"),
					ListUtils.newList("workingDir\t" + SysInfoUtils.getcwd(), // Run from
																																		// current
																																		// directory
							"command\t" + StrUtils.join(cmd, "\t"), "reqMemory\t"
									+ thunkReqMemory, "priority\t" + thunkPriority));
