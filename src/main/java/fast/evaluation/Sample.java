...
public class Sample {
	...
	/**
	 * @author Yun Huang changed the code
	 * @param targets
	...
	public Sample(Dataset dataset, int[] indicesInDataset, double[] weights,
			double[] targets, int[] indicesInParentSample, int size) {
		this.dataset = dataset;
	...
	public Sample getClone() {
		return new Sample(dataset, indicesInDataset, weights, targets,
				indicesInParentSample, size);
	}
	...
	public Sample getRandomSubSample(double rate, Random rnd) {
		if (rate < 1.0) {
			...
			return new Sample(dataset, sampleIndicesInDataset, sampleWeights,
					sampleTargets, sampleIndicesInParentSample, sampleSize);
		}
		else {
	...
	/**
	 * Returns a random subsample that is biased with respect to targets. In other
	 * words it tries to include items with more diverse targets
	 */
	public Sample getRandomTargetBiasedSubSample(double rate, Random rnd) {
		if (rate < 1.0) {
			...
			return new Sample(dataset, sampleIndicesInDataset, sampleWeights,
					sampleTargets, sampleIndicesInParentSample, curSampleSize);
		}
		else {
			Sample result = this.getClone();
			result.indicesInParentSample = Constants.ONE_TWO_THREE_ETC;
	...
	// hy commented
	// public double evaluate(double[] predictions, EvaluationMetric
	// evaluationMetric)
	// throws Exception {
	...
	// EvaluationMetric evaluationMetric, double factor) throws Exception {
	...
	public Sample getOutOfSample(Sample subSample) {
		...
		return new Sample(dataset, oosIndicesInDataset, oosWeights, oosTargets,
				null, oosSize);
	}
}