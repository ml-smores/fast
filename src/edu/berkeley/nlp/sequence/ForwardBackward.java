...
public class ForwardBackward {
            ...
            for (int state = 0; state < numStates; ++state) {
                ...
                for (int beforeState : beforeStates) {
                    double delta = alphas[pos - 1][beforeState]
                            * edgePotentials[pos - 1][beforeState][state]
                ...
                for (int state = 0; state < numStates; ++state) {
                    ...
                    if (nodePotentials[pos][state] > 0.0) {
                        ...
                        for (int nextState : nextStates) {
                            beta += edgePotentials[pos][state][nextState]
    ...
    private void computePosteriors() {
        ...
        for (int t = 0; t < numStates; t++) {
            ...
            double unscaled_posterior = alpha_t * beta_t
                    / (z * nodePotentials[obsLength - 1][t]);
            double posterior_scale = alpha_scale + beta_scale - z_scale;
            double exp_scale = getScaleFactor(posterior_scale);
    ...
    private void probCheck() {
        ...
        for (int pos = 0; pos < obsLength; ++pos) {
            ...
            assert (Math.abs(nodeSum - 1.0) < .001) : "Node Sum: " + nodeSum
                    + " not 1.0 for " + pos;
        }
        for (int i = 0; i + 1 < obsLength; ++i) {
            double edgeSum = 0;
            ...
            assert Math.abs(1.0 - edgeSum) < .001 : "Edge Sum: " + edgeSum + " for "
                    + obsLength;
        }
